...
describe("installing Kudos into a workspace", () => {
  ...
  test("the callback stores the bot token, makes the installer admin and imports members", async () => {
    const state = (await startInstall()).searchParams.get("state")!;
    const res = await t.fetch(`/slack/oauth/callback?code=good&state=${state}`, { redirect: "manual" });
  ...
  test("a forged or reused state is refused", async () => {
    const res = await t.fetch("/slack/oauth/callback?code=good&state=forged", { redirect: "manual" });
  ...
  test("Slack errors are passed back to the landing page", async () => {
    const state = (await startInstall()).searchParams.get("state")!;
    const res = await t.fetch(`/slack/oauth/callback?code=bad&state=${state}`, { redirect: "manual" });
