...
4. **Added by [#102](https://github.com/Silthus/kudos/issues/102): the success metrics stay bounded.** The game's success metrics read one `successStats` row per month (≤ 12), the matching ≤ 12 `workspaceStats` month rows, the member list, and ≤ 12 `memberStats` month rows per departed giver. `tests/success-metrics.test.ts` holds that at 500 members with 100 departed givers under a 1,738-document read cap. The engine keeps `successStats` in the give/revoke transaction (a thank-back lookup per changed kudos row and per kudos back to its giver within 72 h after it); the rebuild computes each month in its `periods` step from the month's kudos plus the five days before it, so a month step reads ~10% more kudos than before. `rollups:verify` checks the row for any `m:` bucket. A workspace backfilled before #102 shows "appears after the next rollup rebuild" until `rollups:backfillAll` (or `rebuildWorkspace`) has run once, which sets `workspaces.successBackfilledAt`.
