...
export function validateRewardInput(input: RewardInput): Required<Pick<RewardInput, "name" | "emoji" | "cost">> & Omit<RewardInput, "name" | "emoji" | "cost"> {
  ...
  if (!name || name.length > b.name) throw new ConvexError(`Give the reward a name of 1–${b.name} characters.`);
  const emoji = input.emoji.trim();
  if (!emoji || emoji.length > b.emoji) throw new ConvexError("Pick an emoji for the reward.");
  ...
  if (input.stock !== undefined && !wholeIn(input.stock, b.stock)) {
