 ...
 * gave. One fruit per five Hog coins claimed, its kind rolled by weight. A fruit is an inventory
 * item with exactly one effect, applied where it belongs (the stall sells sun fruit, moon fruit
 * restores stamina, amber is a Lucky charm charge, star discounts a home stage, heart plants a
 * Super seed). The tree never drops fruit for anything but a claim, so fruit is still kudos.
 */
...
export const FRUITS: Fruit[] = [
  ...
  { id: "heart", name: "Heart fruit", about: "A Super seed: plants a garden plant that starts as a Sapling.", weight: 1, effect: { kind: "superSeed", stage: "sapling" } },
];
...
export function fruitsFor(coinsClaimed: number): number {
  return Math.floor(Math.max(0, coinsClaimed) / FRUIT_PER_COINS);
}

const TOTAL_WEIGHT = FRUITS.reduce((n, f) => n + f.weight, 0);

/** The fruit a claim drops, in drop order; `rand` is a seeded [0, 1) source so a claim replays. */
export function rollFruits(coinsClaimed: number, rand: () => number): FruitId[] {
  return Array.from({ length: fruitsFor(coinsClaimed) }, () => {
