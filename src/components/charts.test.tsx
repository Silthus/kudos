// @vitest-environment happy-dom
import { MotionGlobalConfig } from "motion/react";
import { act } from "react";
import { createRoot, type Root } from "react-dom/client";
import { afterEach, expect, test } from "vitest";
import { Heatmap, LineChart, Sparkline } from "./charts";

(globalThis as { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
MotionGlobalConfig.skipAnimations = true;
// happy-dom has no layout: every chart measures 600 px wide.
globalThis.ResizeObserver = class {
  constructor(private cb: ResizeObserverCallback) {}
  observe() {
    this.cb([{ contentRect: { width: 600 } } as ResizeObserverEntry], this as unknown as ResizeObserver);
  }
  unobserve() {}
  disconnect() {}
};

let root: Root | undefined;
afterEach(() => act(() => root?.unmount()));

function render(ui: React.ReactNode) {
  const host = document.createElement("div");
  root = createRoot(host);
  act(() => root!.render(ui));
  return host;
}

const days = ["2026-09-21", "2026-09-22", "2026-09-23", "2026-09-24", "2026-09-25"];

test("line charts draw square markers and flat fills, never a gradient or a round dot", () => {
  const el = render(
    <>
      <LineChart
        endLabels
        days={days}
        series={[
          { key: "given", label: "Given", color: "var(--color-ember)", values: [1, 3, 2, 5, 4] },
          { key: "received", label: "Received", color: "var(--color-pond)", values: [0, 2, 2, 1, 3] },
        ]}
      />
      <Sparkline values={[1, null, 2, 4]} labels={["a", "b", "c", "d"]} reference={2} format={String} label="Kudos per week" />
    </>,
  );
  expect(el.querySelectorAll("svg").length).toBe(2);
  expect(el.querySelectorAll("linearGradient, radialGradient").length).toBe(0);
  expect(el.querySelectorAll("circle").length).toBe(0);
  expect(el.querySelectorAll("rect[data-marker]").length).toBeGreaterThanOrEqual(4); // 2 end markers + a lone point + the last point
  for (const path of el.querySelectorAll("path[stroke]")) expect(path.getAttribute("stroke-linecap") ?? "square").not.toBe("round");
});

test("the weekday by hour heatmap is drawn in stepped shades of pond, and its numbers are a table", () => {
  const data = Array.from({ length: 7 }, (_, d) => Array.from({ length: 24 }, (_, h) => (d === 1 && h === 10 ? 8 : d === 2 && h === 15 ? 2 : 0)));
  const el = render(<Heatmap data={data} />);
  const cells = [...el.querySelectorAll("[data-cell]")] as HTMLElement[];
  expect(cells).toHaveLength(7 * 24);
  const shades = new Set(cells.map((c) => c.style.background));
  // A handful of steps in one hue: empty hours parchment, busy ones pond.
  expect(shades.size).toBeLessThanOrEqual(5);
  expect(cells.find((c) => c.getAttribute("aria-label") === "Tue 10:00, 8 kudos")!.style.background).toContain("--color-pond");
  expect(cells.every((c) => !c.style.background.includes("lantern"))).toBe(true);
  expect(el.querySelector("summary")?.textContent).toBe("Show data");
  const rows = [...el.querySelectorAll("table tbody tr")].map((r) => r.textContent);
  expect(rows).toHaveLength(7);
  expect(rows[1]).toContain("Tue");
  expect(rows[1]).toContain("8");
  // The table is what screen readers get; the grid of cells is a picture of it.
  expect(el.querySelector("[data-cell]")!.parentElement!.closest("[aria-hidden=true]")).not.toBeNull();
});

test("each heatmap step holds a quarter of the busiest hour: a trickle is the lightest, the busiest the deepest", () => {
  const values = [0, 1, 25, 26, 50, 51, 75, 76, 100];
  const data = Array.from({ length: 7 }, (_, d) => Array.from({ length: 24 }, (_, h) => (d === 0 ? (values[h] ?? 0) : 0)));
  const el = render(<Heatmap data={data} />);
  const cell = (h: number) => el.querySelector(`[aria-label="Mon ${String(h).padStart(2, "0")}:00, ${values[h]} kudos"]`) as HTMLElement;
  // 0 is no step; 1 and 25 share the lightest; 26 and 50 the next; 51 and 75 pond; 76 and 100 deep pond.
  expect(values.map((_, h) => Number(cell(h).dataset.step))).toEqual([0, 1, 1, 2, 2, 3, 3, 4, 4]);
  expect(cell(0).style.background).toContain("parchment-deep");
  expect(cell(6).style.background).toBe("var(--color-pond)");
  expect(cell(8).style.background).toBe("var(--color-pond-deep)");
});
