...
export function SpreePost({ spree, glyph, onJoin }: { spree: Spree; glyph: string; onJoin: (attemptId: string) => Promise<void> }) {
  ...
  return (
    <div>
      <div className="group flex gap-2.5 px-2 py-2 hover:bg-ink/5">
        <Avatar name={spree.author} size={36} />
        <div className="min-w-0 flex-1">
          <div className="text-sm">
            <b className="font-bold">{spree.author}</b>{" "}
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full border border-[#1d9bd1]/60 bg-[#1d9bd1]/15 px-2 py-0.5 text-xs hover:bg-[#1d9bd1]/25"
            >
