...
| **Rebuild** (game) | Playing a member's surviving kudos history through the XP rules and replacing their events (`internal.game.rebuildWorkspace`): the backfill when the game is switched on, the demo year, and the repair tool. Without revokes it writes exactly what the live path wrote; after revokes it re-scores later kudos against the history that survived. | the read-model rollups' rebuild |
